      ...
      <input type="text" name="" v-model="body" placeholder="说的什么吧..." @keyup.enter='recommend'>
        <div class="publish-right">
           ...
           <span @click="collect" > <!-- 收藏 -->
            <i class="iconfont" v-if="shou==0">&#xe60a;</i>
            <i class="iconfont" style="color: orange" v-if="shou==1">&#xe60a;</i>
           </span>
...
export default {
  ...
  methods:{
    getDetailsInfo () {//详情页文章
        ...
        data:{
         do:'xinwen_content',
         newsid:this.myListId,
         weid:this.$store.state.weid,
         openid:this.$store.state.openid,
        },
        }).then(res=>{
            console.log(res.data.datas[0])
            _this.details=res.data.datas[0]
            _this.shou=res.data.datas[0].isshou
            _this.zann=res.data.datas[0].iszan
    ...
    goReplay(index,id){  //回复评论
      console.log(index)
       console.log(id)
      this.$router.push({path:'./replay',query:{id: this.myListId,pid:id,index:index}})
    }  
